...
class ReleaseManifestTest(unittest.TestCase):
    ...
    def test_generates_hash_pinned_homebrew_and_scoop_manifests(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            inventory = json.loads((output / "smorg-assets.json").read_text())
            ...
            self.assertEqual(inventory["assets"]["windows-x86_64"]["sha256"], expected)
            self.assertEqual(set(inventory["assets"]), set(manifests.PLATFORMS))
