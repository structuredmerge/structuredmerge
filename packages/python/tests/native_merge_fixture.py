...
def run_json_common(operation, dialect, sources, git_options=None, family="json"):
    ...
    core.register_language_pack_parser(provider_id, family if family in ("bash", "go") else ("json" if dialect == "json" else "json5"))
    try:
        ...
        request = core.OperationRequest(schema=original.schema, request_id=original.request_id,
            ...
            provider_selection=core.MergeProviderSelection(provider_id=f"kernel.{family}" if family in ("bash", "go") else ("kernel.json" if git_options is None else "kernel.git.json"), family=family, dialect=dialect,
                profile_id=f"kernel.{family}.owners.v1" if family in ("bash", "go") else ("kernel.json.nested.v1" if git_options is None else "kernel.git.json.v1"), required_capabilities=[operation], extra={}),
