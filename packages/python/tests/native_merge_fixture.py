...
def run_json_common(operation, dialect, sources, git_options=None, family="json"):
    ...
    core.register_language_pack_parser(provider_id, dialect if family in ("bash", "go", "rust", "typescript") else ("json" if dialect == "json" else "json5"))
    try:
        ...
        request = core.OperationRequest(schema=original.schema, request_id=original.request_id,
            ...
            provider_selection=core.MergeProviderSelection(provider_id=f"kernel.{family}" if family in ("bash", "go", "rust", "typescript") else ("kernel.json" if git_options is None else "kernel.git.json"), family=family, dialect=dialect,
                profile_id=f"kernel.{family}.owners.v1" if family in ("bash", "go", "rust", "typescript") else ("kernel.json.nested.v1" if git_options is None else "kernel.git.json.v1"), required_capabilities=[operation], extra={}),
...
def run_typescript_common_analyze(dialect, source):
    return run_json_common("analyze", dialect, [source], family="typescript")


def run_typescript_common_diff(dialect, before, after):
    return run_json_common("diff2", dialect, [before, after], family="typescript")


def run_typescript_common_merge2(dialect, incoming, current):
    return run_json_common("merge2", dialect, [incoming, current], family="typescript")


def run_typescript_common_merge3(dialect, base, ours, theirs):
    return run_json_common("merge3", dialect, [base, ours, theirs], family="typescript")
