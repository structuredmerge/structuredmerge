//! Current-preferred Rust declaration insertion over native ownership.
//! Use declarations and inner documentation stay at module scope. Byte scans
...
/// Empty/use-only documents are directional endpoints, not wider merge3 support.
pub fn owners(parsed: &ParsedResult) -> Result<SourcePreservingOwnerDocument, String> {
    if !parsed.backend.languages.iter().any(|language| language == "rust") {
        return Err("Rust directional analysis requires a Rust parser".into());
    }
    let nodes = parsed.normalized_nodes()?;
    let index = tree_haver::NormalizedTreeIndex::new(&nodes)?;
    let root =
        index.root(parsed.document.output().root_id.as_deref().ok_or("missing Rust root")?)?;
    let mut has_owner = false;
    for node in index.children(root) {
        if super::supported_analysis_declaration(&node.kind) {
            has_owner = true;
        } else if node.role != NodeRole::Comment && node.kind != "use_declaration" {
            return Err("unsupported Rust directional top-level declaration".into());
        }
    }
    if has_owner {
...
fn layout(
    ...
    let mut result = Layout { uses: vec![], ranges: BTreeMap::new(), empty_offset: 0 };
    let mut previous_end = 0;
    for node in &children {
        let is_use = node.kind == "use_declaration";
        let is_inner_doc = node.role == NodeRole::Comment
            && index.children(node).iter().any(|child| child.kind == "inner_doc_comment_marker");
        let owner = owners_by_span.get(&(node.span.range.start_byte, node.span.range.end_byte));
        if !is_use && !is_inner_doc && owner.is_none() {
            continue;
        }
        ...
        {
            return Err(
                "Rust directional insertion requires standalone native declaration lines".into()
            );
        }
        if is_use {
            result.uses.push(node.source_fragment.clone());
        }
        if is_use || is_inner_doc {
            // Module-scoped barriers never travel with an inserted declaration.
            result.empty_offset = line_end;
        } else if let Some(owner) = owner {
            result.ranges.insert(
                owner.id.clone(),
                ByteRange { start_byte: previous_end, end_byte: line_end },
            );
        }
...
/// Exact use declarations must agree when adding owners. This is not import/name
/// resolution, attribute/macro expansion or semantic dependency reconciliation.
pub fn plan_insertions(
    ...
    if owners(incoming_parse)? != *incoming || owners(current_parse)? != *current {
        return Err("Rust directional ownership differs from native facts".into());
    }
    ...
    if incoming_layout.uses != current_layout.uses {
        return Err("Rust declaration insertion requires identical use declarations".into());
    }
    let shared: Vec<_> =
        incoming.owners.iter().filter_map(|owner| positions.get(owner.id.as_str())).collect();
    if shared.windows(2).any(|pair| pair[0] >= pair[1]) {
        return Err("Rust shared anchors are reordered".into());
    }
    let mut result = vec![];
    for (index, owner) in incoming.owners.iter().enumerate() {
        ...
        {
            return Err("Rust directional insertion requires explicit newline boundaries".into());
        }
